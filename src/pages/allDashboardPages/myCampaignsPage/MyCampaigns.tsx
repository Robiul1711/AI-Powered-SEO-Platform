...
import { Megaphone, Loader2, ChevronLeft, ChevronRight, Eye, LayoutGrid, Search } from 'lucide-react';
...
const MyCampaigns = () => {
  ...
  const filteredCampaigns = allCampaigns.filter((campaign: any) => {
    ...
    const status = (campaign.status || campaign.payment_status || "Active").toLowerCase();
    
    const matchesSearch = id.includes(search) || name.includes(search) || status.includes(search);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="mt-3 text-gray-500 font-orbitron text-xs  animate-pulse">Loading Campaigns...</p>
      </div>
    );
  }
    ...
    <div className="font-inter pb-8">
      <div className="bg-[#1A1A1A] border border-white/5 rounded-2xl p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl font-medium text-white">My Campaigns</h2>
            ...
            <tbody className="text-sm">
              {campaigns.length > 0 ? (
                campaigns.map((campaign: any, index: number) => {
                  const name = campaign.campaign_tier?.campaign?.title || campaign.pricing_plan?.name || "Campaign";
                  const date = new Date(campaign.created_at).toLocaleDateString('en-US', {
                    year: 'numeric',
                    month: 'short',
                    day: '2-digit'
                  });
                  const status = campaign.status || campaign.payment_status || "Active";
                    ...
                    <tr key={index} className="border-b border-gray-800 hover:bg-white/5 transition-colors">
                      ...
                      <td className="py-2.5 px-4 text-gray-300">
                        <div className="flex flex-col gap-1">
                          <span>{name}</span>
                          <span className="text-xs text-gray-500">CMP-{campaign.id}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-2.5 px-4">
                        <Link to={`/dashboard/my-campaigns/${campaign.id}`} className="bg-gradient-to-r from-[#9056e3] to-[#b47afc] hover:from-[#8046d3] hover:to-[#a46aec] text-white px-4 py-1.5 rounded-md transition-all inline-flex items-center text-[13px] font-medium shadow-md shadow-[#AC6CFF]/20">
                          View
                        </Link>
                      </td>
                    </tr>
                ...
                <tr>
                  <td colSpan={4} className="py-8 text-center text-gray-500 text-sm">No campaigns found.</td>
                </tr>
              )}
            </tbody>
          </table>
